...
static int domain_send_s3resume(xc_interface *xch, unsigned int domid)
{
	return xc_set_hvm_param(xch, domid, HVM_PARAM_ACPI_S_STATE, 0);
}

static int domain_set_timer_mode(xc_interface *xch, unsigned int domid, int mode)
{
	return xc_set_hvm_param(xch, domid,
                            HVM_PARAM_TIMER_MODE, (unsigned long) mode);
}

static int domain_set_hpet(xc_interface *xch, unsigned int domid, int hpet)
{
	return xc_set_hvm_param(xch, domid, HVM_PARAM_HPET_ENABLED, (unsigned long) hpet);
}
static int domain_set_vpt_align(xc_interface *xch, unsigned int domid, int vpt_align)
{
...
CAMLprim value stub_xc_domain_set_timer_mode(value xch, value id, value mode)
{
	...
	ret = domain_set_timer_mode(_H(xch), _D(id), Int_val(mode));
	if (ret < 0)
		failwith_xc(_H(xch));
	CAMLreturn(Val_unit);
}
...
CAMLprim value stub_xc_domain_set_hpet(value xch, value id, value mode)
{
	...
	ret = domain_set_hpet(_H(xch), _D(id), Int_val(mode));
	if (ret < 0)
		failwith_xc(_H(xch));
	CAMLreturn(Val_unit);
}
...
CAMLprim value stub_xc_domain_set_vpt_align(value xch, value id, value mode)
{
	...
	ret = domain_set_vpt_align(_H(xch), _D(id), Int_val(mode));
	if (ret < 0)
		failwith_xc(_H(xch));
	CAMLreturn(Val_unit);
}
...
{
	CAMLparam2(xch, domid);
	domain_send_s3resume(_H(xch), _D(domid));
	CAMLreturn(Val_unit);
}
